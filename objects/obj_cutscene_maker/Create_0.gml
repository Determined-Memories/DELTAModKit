...
step_current = 0;
step_max = 0;
make_effect_timer = false;

for (i = 0; i < 10; i++)
{
    actor_startx[i][0] = camerax() + (camerawidth() / 2);
    actor_starty[i][0] = cameray() + (cameraheight() / 2);
    actor_endx[i][0] = camerax() + (camerawidth() / 2);
    actor_endy[i][0] = cameray() + (cameraheight() / 2);
