...
if (keyboard_check(vk_shift) && keyboard_check(vk_control) && keyboard_check_pressed(ord("P"))) {
    ...
    with (all) {
        ...
        if (persistent == true) writestring += " [persistent object]";
        
        if (visible == false) writestring += " [not visible]"; // !!NO LONGER WORKS DUE TO WITH IGNORING NON VISIBLE OBJECTS!!
...
if (keyboard_check(vk_shift) && keyboard_check(vk_control) && keyboard_check_pressed(ord("G")))
{
    ...
    if global.chemg_god_mode snd_play_pitch(snd_swing, 1.5);
    if global.chemg_god_mode snd_play_pitch(snd_swing, 0.5);
}

if global.chemg_god_mode
    for (var i = 0; i < array_length(global.maxhp); i++)
        if (global.hp[i] < global.maxhp[i])
            global.hp[i]++;

if (keyboard_check(vk_shift) && keyboard_check(vk_control) && keyboard_check_pressed(ord("B")))
{
    with (obj_solidblock) visible = true;
...
if (keyboard_check(vk_shift) && keyboard_check(vk_control) && keyboard_check_pressed(ord("W")))
{
    ...
    if (windowswitcher == false) window_set_size(1280, 960);
    
    if (windowswitcher == true) window_set_size(640, 480);
...
if (keyboard_check(vk_shift) && keyboard_check(vk_alt) && keyboard_check_pressed(ord("M"))) // NEW
{
    with obj_time showmodkitmessage = !showmodkitmessage
}
}